    ...
    const fetchOutdoorPlants = async () => {
      try {
        const responses = await Promise.all([
          axios.get("/api/trefle/plants", { params: { page: 3 } }),
          axios.get("/api/trefle/plants", { params: { page: 4 } }),
        ]);
        ...
        console.error("Outdoor API error:", fetchError.response?.data || fetchError.message);
        setError(
          `Failed to fetch outdoor plants. Status: ${fetchError.response?.status || "unknown"
          }`
        );
      } finally {
        setLoading(false);
  ...
  return (
    <section className="min-h-screen bg-white px-6 py-12">
      <div className="mx-auto max-w-6xl">
        ...
        {loading && <p className="text-center text-gray-600">Loading outdoor plants...</p>}
